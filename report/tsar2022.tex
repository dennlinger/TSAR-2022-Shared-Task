...
\noindent We find that our system also outperforms all competing submitted approaches in the shared task; result comparisons can be found in \Cref{tab:spanish} and \Cref{tab:portuguese}, respectively.
...
\begin{table*}
	...
	\end{tabular}
	\caption{Results on the Portuguese language test set of the TSAR-2022 shared task, ranked by \emph{ACC@1} scores. Listed are our own results (\emph{Ensemble} and  \emph{Single}), the two best-performing competing systems~(\emph{MANTIS} and \emph{UoM\&MMU}), as well as provided baselines (\emph{LSBert}~\cite{qiang-etal-2020-lexical} and TUNER~\cite{ferres-etal-2017-adaptable}).}
